    ...
    if webcam:
        ...
        with mss.mss() as sct:
            ...
            while True:
                ...
                im_resized = cv2.resize(img, (imgsz[0], imgsz[1]))  # Resize for YOLO model
                im_resized = im_resized[:, :, ::-1].transpose(2, 0, 1)  # HWC to CHW format
                im_resized = np.ascontiguousarray(im_resized)

                # Run detection on the resized screen feed
                im = torch.from_numpy(im_resized).to(model.device).float()
                im /= 255.0  # Normalize
                if len(im.shape) == 3:
                ...
                im0_resized = cv2.resize(im0, (1920, 1080)) 
                cv2.imshow("Screen Detection", im0_resized)
